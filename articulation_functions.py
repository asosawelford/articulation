...
def bark(f):
    ...
    for i in range(0, len(f)):
        # Bark scale values
        b.append(13*(math.atan(x[i]))+3.5*(math.atan(x2[i])))
    return b
...
def fftsolp(x, nfft):
    ...
    rowindex = rowindex.T
    d = np.ones((nwind, ncol), dtype=int)
...
def extract_transitions(segments, fs, size_frameS, size_stepS, nB=22, nMFCC=12, nfft=2048):
    ...
    for j in range(len(segments)):
        if (len(segments[j]) > size_frameS):
            ...
            for iF in range(nF):
                frames.append(np.hamming(
                    size_frameS)*segments[j][int(iF*size_stepS):int(iF*size_stepS+size_frameS)])
    ...
    for j in range(len(frames)):
        ...
        MFCC[j, :] = pysptk.sptk.mfcc(
            frame_act, order=nMFCC, fs=fs, alpha=0.97, num_filterbanks=32, cepslift=22, use_hamming=True)
    return BarkEn, MFCC
